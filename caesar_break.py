...
# loweronly = ''.join(onlyletters).lower()
...
encrypted = list(map(lambda x: shiftBy(x, caesar_key), loweronly))
...
for ascii in range(ord('a'), ord('a')+26):
    enc1 = encrypted.count(chr(ascii))
    frequency[chr(ascii)] = float(enc1)/len(loweronly)
